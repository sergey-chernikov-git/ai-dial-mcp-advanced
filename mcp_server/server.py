...
async def handle_mcp_request(
    ...
    if not _validate_accept_header(accept):
        ...
        return Response(
            status_code=406,
            content=json.dumps(MCPResponse(
                id="server-error",
                error=ErrorResponse(code=-32600, message="Client must accept both application/json and text/event-stream")
    ...
    if request.method == "initialize":
        mcp_response, mcp_session_id = mcp_server.handle_initialize(request)
        response.headers[MCP_SESSION_ID_HEADER] = mcp_session_id
    else:
        ...
        if not session.ready_for_operation:
            error_response = MCPResponse(
                id="server-error",
                error=ErrorResponse(code=-32600, message="Session not ready for operation")
            )
            return Response(
                status_code=400,
                content=error_response.model_dump_json(),
